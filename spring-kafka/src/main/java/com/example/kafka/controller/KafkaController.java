...
public class KafkaController {
    ...
    // Add another consumer to a group
    // curl -X POST "http://localhost:8081/api/kafka/consumer/add?groupId=group-1&topic=rebalance-test"
    @PostMapping("/consumer/add")
    public ResponseEntity<?> addConsumer(
            @RequestParam String groupId,
            @RequestParam(defaultValue = DEFAULT_TOPIC) String topic) {
        String consumerId = consumerService.addConsumer(groupId, topic);
        return ResponseEntity.ok(Map.of("status", "added", "consumerId", consumerId, "groupId", groupId, "topic", topic,
                "tip", "Watch logs for [REBALANCE] messages to see partition assignment"));
    }

    // Stop a single consumer
    // curl -X POST "http://localhost:8081/api/kafka/consumer/stop-single?consumerId=group-1-abcd"
    @PostMapping("/consumer/stop-single")
    public ResponseEntity<?> stopSingleConsumer(@RequestParam String consumerId) {
        consumerService.stopSingleConsumer(consumerId);
        return ResponseEntity.ok(Map.of("status", "stopped", "consumerId", consumerId));
    }

    // Stop a consumer group (stops all consumers in the group)
